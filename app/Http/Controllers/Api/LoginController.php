...
class LoginController extends Controller{
    public function Test(Request $request){
        return ['code'=>0,'data'=>"","msg"=>'user has been created'];
    }
    ...
    public function upload_photo (Request $request){
        ...
        try{
           ...
           $fullFileName = uniqid().'.'.$extension;  // doing unique name with extension
           $timeDir = date("Ymd");
           $file->storeAs($timeDir, $fullFileName, ["disk"=> "public"]); 
           $url = env("APP_URL")."/uploads".$timeDir.'/'.$fullFileName;
           return ["code"=>0,"date"=>$url,"msg"=>"success image upload"];
        }catch(Exception $e){
            return ["code"=>-1,"date"=>"","msg"=>"error uploading image"];
        }
    }
}